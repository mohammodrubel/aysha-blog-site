...
const JapaneseDetail = () => {
    ...
    const commentsSubmit = e =>{
        ...
        fetch('https://obscure-sierra-80533.herokuapp.com/comments',{
            method:'POST',
            headers:{
    ...
    const hendleDelete = id =>{
        ...
        if(proced){
            const url = `https://obscure-sierra-80533.herokuapp.com/comments/${id}`;
            fetch(url,{
                method:'DELETE',
            })
            .then(res => res.json())
    ...
    return (
        ...
        <h2 style={{color:'#090924',textAlign:'center',marginTop:'30px'}}>{headline}</h2>
            <Col className="col-sm-12 text-center"style={{margin:'60px 0px'}}>
                <div>
                     <hr />
                    ...
                    <p className='discription' style={{lineHeight:'35px'}}>{discription}{discription}</p>
                </div>
